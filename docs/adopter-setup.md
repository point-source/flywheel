...
> **`.releaserc.json` precedence.** Flywheel generates `.releaserc.json` from `.flywheel.yml` at runtime and writes it before invoking `semantic-release`. **If you commit a `.releaserc.json` to your repo, Flywheel leaves it alone** — yours wins. Trade-off: you must keep its `branches` and `tagFormat` in sync with `.flywheel.yml` yourself. Use a committed file only when you need plugins or behaviors not generated by default (e.g. `@semantic-release/exec` for non-Node version files like `pubspec.yaml`, `Cargo.toml`, `pyproject.toml`). For canonical recipes per ecosystem, see [`docs/adopter-recipes.md`](./adopter-recipes.md). Only `.releaserc.json` is honored — Flywheel does not detect `.releaserc.js`, `.releaserc.yml`, or `release.config.js`.
